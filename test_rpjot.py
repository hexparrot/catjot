...
# ---------------------------------------------------------------------------
# 7. run_tool_loop -- full loop, live LLM
# ---------------------------------------------------------------------------


class TestRunToolLoop(unittest.TestCase):
    """run_tool_loop must always return a dict with a non-empty content string."""

    SYSTEM = (
        "You are the narrator of a text RPG. "
        "Use tools as needed, then return a narrative response to the player."
    )
    ...
    def _run(self, user_text, max_iterations=5):
        messages = _base_messages(self.SYSTEM)
        messages.append(self.engine.build_user_message(user_text))
        return self.engine.run_tool_loop(messages, max_iterations=max_iterations)

    def test_simple_action_returns_dict(self):
        self.assertIsInstance(self._run("I enter the manor."), dict)

    def test_response_has_content_key(self):
        self.assertIn("content", self._run("I look around."))

    def test_narrative_is_nonempty(self):
        response = self._run("I look around the room carefully.")
        self.assertTrue(response.get("content", "").strip())

    def test_inquiry_returns_narrative(self):
        response = self._run("Who is here with me?")
        self.assertTrue(response.get("content", "").strip())

    def test_narrative_is_not_raw_json(self):
        """The final narrative must be prose, not a bare JSON blob."""
        response = self._run("I look around the room carefully.")
        content = response.get("content", "")
        try:
            json.loads(content)
            self.fail(f"Narrative is raw JSON, not prose: {content!r}")
        except (json.JSONDecodeError, ValueError):
            pass  # expected -- it is prose

    def test_think_tags_stripped_from_narrative(self):
        response = self._run("I sit down and rest.")
        content = response.get("content", "")
        _, clean = RPJotEngine.strip_think_tags(content)
        self.assertTrue(clean.strip(), "Narrative is empty after stripping think tags.")

    def test_loop_terminates_within_max_iterations(self):
        response = self._run("I wander aimlessly.", max_iterations=3)
        self.assertIn("content", response)

    def test_max_iterations_one_still_returns_content(self):
        """Even with a cap of 1, the loop must return something."""
        response = self._run("I look around.", max_iterations=1)
        self.assertIn("content", response)

    def test_repeated_calls_all_return_content(self):
        for i in range(3):
            with self.subTest(run=i):
                response = self._run("I pause and listen.")
                self.assertTrue(response.get("content", "").strip())
...
class TestToolRegistry(unittest.TestCase):
    ...
    def test_followup_injected_into_messages(self):
        """run_tool_loop must append followup_instruction as a user message."""
        injected = []
        messages = [{"role": "system", "content": "test"}]
        tool_result = json.dumps(
            {
                "people": ["player"],
                "followup_instruction": "SENTINEL_FOLLOWUP",
            }
        )

        self.engine._dispatch = lambda name, args: tool_result

        fake_tool_call = [
            {
                "id": "test-id",
                "function": {"name": "get_people_present", "arguments": "{}"},
            }
        ]
        fake_with_tools = {
            "role": "assistant",
        ...
        fake_narrative = {"role": "assistant", "content": "The room is empty."}

        call_count = [0]

        def fake_call_llm(msgs, tools=None, tool_choice=None, **kwargs):
            call_count[0] += 1
            if call_count[0] == 1:
                return fake_with_tools
        ...
        try:
            self.engine.run_tool_loop(messages)
        finally:
            rpjot_module.call_llm = original

        user_contents = [m["content"] for m in messages if m.get("role") == "user"]
        self.assertTrue(
            any("SENTINEL_FOLLOWUP" in c for c in user_contents),
            f"followup not found in messages: {user_contents}",
        )
...
# ---------------------------------------------------------------------------
# 17. _tool_record_knowledge -- live LLM integration tests
# ---------------------------------------------------------------------------


class TestRecordKnowledgeLive(unittest.TestCase):
    """Live LLM tests: narrator uses record_knowledge tool to store a secret,
    then we verify POV retrieval honours knowledge isolation.

    The scenario: alice, bob, and carol are in a room.  alice whispers a unique
    passphrase (ZEPHYR-9) to bob.  carol is present but not privy.  We prompt
    the LLM narrator to call record_knowledge with the appropriate witnesses so
    that only alice and bob can recall the passphrase later.
    """

    SECRET = "ZEPHYR-9"
    OBSERVABLE = "alice leaned close to bob and whispered privately"

    SYSTEM = (
        "You are a game master narrating a text RPG. "
        "Use the record_knowledge tool whenever information is shared privately. "
        "Always call record_knowledge before writing your narrative response. "
        "After calling the tool, write a brief in-character narrative."
    )
    ...
    def setUpClass(cls):
        ...
        messages = _base_messages(cls.SYSTEM)
        prompt = (
            f"alice leans close to bob and whispers the passphrase '{cls.SECRET}' "
            f"to him privately, making sure carol cannot hear. "
            f"Record this secret knowledge with witnesses alice and bob only. "
            f"The observable act is: '{cls.OBSERVABLE}'."
        )
        messages.append(cls._engine.build_user_message(prompt))
        cls._response = cls._engine.run_tool_loop(messages)
        cls._alice_msgs = [n.message for n in cls._engine.gather_pov_context("alice")]
        cls._bob_msgs = [n.message for n in cls._engine.gather_pov_context("bob")]
        cls._carol_msgs = [n.message for n in cls._engine.gather_pov_context("carol")]
        cls._alice_ctx = cls._engine.render_context(
            cls._engine.gather_pov_context("alice")
        )
        cls._bob_ctx = cls._engine.render_context(cls._engine.gather_pov_context("bob"))
        cls._carol_ctx = cls._engine.render_context(
            cls._engine.gather_pov_context("carol")
        )
    ...
    def test_record_knowledge_registered(self):
        names = [s["function"]["name"] for s in self._engine._tool_schemas]
        self.assertIn("record_knowledge", names)

    # -- LLM calls the tool and persists notes ------------------------------

    def test_run_tool_loop_returns_narrative(self):
        self.assertTrue(
            self._response.get("content", "").strip(),
            "LLM must return a non-empty narrative",
        )

    def test_secret_written_to_notefile(self):
        with open(TMP_CATNOTE) as f:
            raw = f.read()
        self.assertIn(self.SECRET, raw, "secret must appear somewhere in the note file")

    # -- knowledge isolation after LLM call ---------------------------------

    def test_alice_knows_secret(self):
        self.assertTrue(
            any(self.SECRET in m for m in self._alice_msgs),
            f"alice must know the secret after record_knowledge; msgs={self._alice_msgs}",
        )

    def test_bob_knows_secret(self):
        self.assertTrue(
            any(self.SECRET in m for m in self._bob_msgs),
            f"bob must know the secret after record_knowledge; msgs={self._bob_msgs}",
        )

    def test_carol_does_not_know_secret(self):
        self.assertFalse(
            any(self.SECRET in m for m in self._carol_msgs),
            f"carol must NOT know the secret; msgs={self._carol_msgs}",
        )

    # -- observable act isolation ------------------------------------------

    def test_carol_knows_exchange_occurred(self):
        self.assertTrue(
            any(self.OBSERVABLE in m for m in self._carol_msgs),
            "carol must see the observable act even without knowing the secret",
        )

    def test_alice_sees_observable_act(self):
        self.assertTrue(
            any(self.OBSERVABLE in m for m in self._alice_msgs),
            "alice must also see the observable act note",
        )

    # -- gather_pov_context used directly, not just via scene map ----------

    def test_pov_context_carol_lacks_secret(self):
        self.assertNotIn(
            self.SECRET,
            self._carol_ctx,
            "carol's rendered POV context must not contain the secret",
        )

    def test_pov_context_bob_has_secret(self):
        self.assertIn(
            self.SECRET,
            self._bob_ctx,
            "bob's rendered POV context must contain the secret",
        )
